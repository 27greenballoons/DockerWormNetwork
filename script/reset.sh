...
echo -e "${BLUE}[*] DockerWormNetwork Lab Reset Tool${NC}"
...
if [ ! -d "$BASE_DIR/docker" ]; then
    ...
    echo -e "${RED}[!] Make sure reset.sh is in the script/ folder${NC}"
    exit 1
fi
...
# Phase 3: Destroy Containers
echo -e "${BLUE}[*] Phase 3: Destroying containers (kills background worms)...${NC}"
...
# Phase 4: Clean Data
echo -e "${BLUE}[*] Phase 4: Purging shared data and logs...${NC}"
...
echo -e "${BLUE}[*] Phase 5: Rebuilding and deploying baseline...${NC}"
...
echo -e "  • Reset again:      ${NC}sudo $BASE_DIR/script/reset.sh"
echo ""
